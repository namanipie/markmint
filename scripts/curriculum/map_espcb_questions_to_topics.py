"""
Deterministic, additive, auditable ESPCB question-to-topic mapping CLI.
Operates on local database only; preserves all existing mappings; never overwrites conflicts.

Usage:
  python -m scripts.curriculum.map_espcb_questions_to_topics --dry-run
  python -m scripts.curriculum.map_espcb_questions_to_topics --apply
"""
import argparse
import os
import sys
...
def run_espcb_mapping(
    ...
    course = c.execute("SELECT id, name, code FROM courses WHERE id = 18").fetchone()
    if not course:
        conn.close()
        raise ValueError("Course ID 18 (Electronic System and PCB Design) not found.")
    ...
    for p in proposals:
        ...
        assert tid in all_topic_ids, f"Topic {tid} does not belong to Course 18!"
...
def print_audit_report(results: Dict[str, Any]):
    print("=" * 78)
    print(" COURSE 18 (ESPCB) QUESTION-TO-TOPIC MAPPING QUALITY AUDIT")
    print("=" * 78)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Course 18 (ESPCB) Question-to-Topic Mapping CLI")
    ...
    apply = args.apply
    res = run_espcb_mapping(apply_changes=apply)
