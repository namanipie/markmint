"""
Deterministic, additive, auditable OODP question-to-topic mapping CLI.
Operates on local database only; preserves all existing mappings; never overwrites conflicts.

Usage:
  python -m scripts.curriculum.map_oodp_questions_to_topics --dry-run
  python -m scripts.curriculum.map_oodp_questions_to_topics --apply
"""
import argparse
import os
import sys
...
def run_oodp_mapping(
    ...
    course = c.execute("SELECT id, name, code FROM courses WHERE id = 17").fetchone()
    if not course:
        conn.close()
        raise ValueError("Course ID 17 (Object Oriented Design and Programming) not found.")
    ...
    for p in proposals:
        ...
        assert tid in all_topic_ids, f"Topic {tid} does not belong to Course 17!"
...
def print_audit_report(results: Dict[str, Any], questions: List[sqlite3.Row] = None):
    print("=" * 78)
    print(" COURSE 17 (OODP) QUESTION-TO-TOPIC MAPPING QUALITY AUDIT")
    print("=" * 78)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Course 17 (OODP) Question-to-Topic Mapping CLI")
    ...
    apply = args.apply
    res = run_oodp_mapping(apply_changes=apply)
