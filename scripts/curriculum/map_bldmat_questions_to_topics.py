"""
Deterministic, additive, auditable Building Materials in the Built Environment question-to-topic mapping CLI.
Operates on local database only; preserves all existing mappings; never overwrites conflicts.

Usage:
  python -m scripts.curriculum.map_bldmat_questions_to_topics --dry-run
  python -m scripts.curriculum.map_bldmat_questions_to_topics --apply
"""
import argparse
import os
import sys
...
def run_bldmat_mapping(
    ...
    course = c.execute("SELECT id, name, code FROM courses WHERE id = 23").fetchone()
    if not course:
        conn.close()
        raise ValueError("Course ID 23 (Building Materials in the Built Environment) not found.")
    ...
    for prop in proposals:
        ...
        if prop.confidence in ("HIGH", "MEDIUM") and mapped_tid:
            if mapped_tid not in all_topic_ids:
                raise ValueError(f"Topic {mapped_tid} does not belong to Course 23!")
    ...
    print("=" * 70)
    print("COURSE 23: BUILDING MATERIALS QUESTION-TOPIC MAPPING AUDIT")
    print("=" * 70)
    print(f"Total questions in Course 23:        {len(questions)}")
    print(f"Topics available in Course 23:       {len(all_topic_ids)}")
    ...
    if apply_changes:
        ...
        total_mapped = c.execute("""
            ...
            WHERE e.course_id = 23
        """).fetchone()[0]
        print(f"Post-apply total mapped questions for Course 23: {total_mapped}")
...
def main():
    parser = argparse.ArgumentParser(description="Map Building Materials questions to topics.")
    ...
    run_bldmat_mapping(db_path=args.db_path, apply_changes=args.apply)
