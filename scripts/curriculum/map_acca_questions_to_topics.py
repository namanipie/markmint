"""
Deterministic, additive, auditable ACCA question-to-topic mapping CLI.
Operates on local database only; preserves all existing mappings; never overwrites conflicts.

Usage:
  python -m scripts.curriculum.map_acca_questions_to_topics --dry-run
  python -m scripts.curriculum.map_acca_questions_to_topics --apply
"""
import argparse
import os
import sys
...
def run_acca_mapping(
    ...
    course = c.execute("SELECT id, name, code FROM courses WHERE id = 16").fetchone()
    if not course:
        conn.close()
        raise ValueError("Course ID 16 (Advanced Calculus and Complex Analysis) not found.")
    ...
    mappable = [p for p in proposals if p.confidence in ("HIGH", "MEDIUM") and p.topic_id is not None]

    for p in mappable:
        if p.topic_id not in all_topic_ids:
            conn.close()
            raise ValueError(f"Safety Violation: Topic ID {p.topic_id} does not belong to Course 16!")
    ...
    for p in mappable:
        t_info = topic_map.get(p.topic_id, {})
        u_num = t_info.get("unit_number", 0)
        u_name = t_info.get("unit_name", "Unknown Unit")
        t_name = t_info.get("topic_name", "Unknown Topic")
    ...
    overloaded_topics = [
        (k, count, f"{count / len(mappable) * 100:.1f}%")
        for k, count in topic_distribution.items()
        if len(mappable) > 0 and (count / len(mappable)) > 0.20
    ]
    ...
    return {
        ...
        "coverage_pct": round(len(mappable) / len(questions) * 100, 2) if questions else 0.0,
...
def print_audit_report(res: Dict[str, Any]):
    print("=" * 78)
    print(" COURSE 16 (ACCA) QUESTION-TO-TOPIC MAPPING QUALITY AUDIT")
    print("=" * 78)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Map Course 16 ACCA questions to topics.")
    parser.add_argument("--dry-run", action="store_true", help="Run audit without writing.")
    parser.add_argument("--apply", action="store_true", help="Apply proposed mappings to DB.")
    parser.add_argument("--db-path", default=DEFAULT_DB_PATH, help="Path to database.")
    args = parser.parse_args()

    apply_flag = args.apply and not args.dry_run
    res = run_acca_mapping(db_path=args.db_path, apply_changes=apply_flag)
