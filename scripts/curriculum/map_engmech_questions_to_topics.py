"""
Deterministic, additive, auditable Engineering Mechanics question-to-topic mapping CLI.
Operates on local database only; preserves all existing mappings; never overwrites conflicts.

Usage:
  python -m scripts.curriculum.map_engmech_questions_to_topics --dry-run
  python -m scripts.curriculum.map_engmech_questions_to_topics --apply
"""
import argparse
import os
import sys
...
def run_engmech_mapping(
    ...
    course = c.execute("SELECT id, name, code FROM courses WHERE id = 21").fetchone()
    if not course:
        conn.close()
        raise ValueError("Course ID 21 (Engineering Mechanics) not found.")
    ...
    for p in proposals:
        ...
        assert tid in all_topic_ids, f"Topic {tid} does not belong to Course 21!"
...
def print_audit_report(results: Dict[str, Any]):
    print("=" * 78)
    print(" COURSE 21 (ENGMECH) QUESTION-TO-TOPIC MAPPING QUALITY AUDIT")
    print("=" * 78)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Course 21 (ENGMECH) Question-to-Topic Mapping CLI")
    ...
    apply = args.apply
    res = run_engmech_mapping(apply_changes=apply)
